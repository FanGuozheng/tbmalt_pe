...
def test_interp():
    ...
    path3 = '/home/gz_fan/Downloads/test/work/twoparams/test/HCNO/test/H-C.skf'
    sk, sk3 = Skf.read(path), Skf.read(path3)
    with h5py.File('../train/tvcr.h5', 'r') as f:

        H = f['H-H']['integrals']['H'][()][0]
        HC = torch.from_numpy(f['H-C']['integrals']['H'][()][0]).transpose(-2, 0)
        step = f['H-H']['integrals']['step'][()]
        n_grid = f['H-H']['integrals']['count'][()]
        hs_grid = torch.arange(1, n_grid) * step

        N, bpred = 20, []
        _g = grid.numpy()

        # 1. Linear interpolation
        interp = MultiVarInterp(
            [grid, grid, grid, grid, hs_grid], torch.from_numpy(H)[:, :, :, :, 0])
        y = interp(torch.tensor([3.5, 3.5, 3.5, 3.5]).repeat(N, 1),
                   torch.arange(1, N + 1) * step)
        ref = sk.hamiltonian[(0, 0)][0]
        ref3 = sk3.hamiltonian[(0, 0)][0]
        ...
        _H = torch.from_numpy(H).transpose(-2, 0)
        # for ih in _H:
        bspline = TensorSpline(_H, grid, grid, grid, grid, torch.arange(55) * 0.2)
        bpred2 = bspline(
            torch.tensor([3.5]).repeat(N-1), torch.tensor([2.25]).repeat(N-1),
            torch.tensor([4.5]).repeat(N-1), torch.tensor([3.75]).repeat(N-1),
            torch.arange(1, N) * 0.2)

        # interpolation for H-C grid points
        N1, N2, N3, N4, Nd = 2, 3, 4, 5, 1
        distances = torch.arange(HC.shape[-1]) * 0.2
        print('HC', HC.shape, HC[0, N1, N2, N3, N4, Nd], HC[0, :, N2, N3, N4, Nd],
              '\n grid', grid, torch.arange(10) * 0.2)
        bspline3 = TensorSpline2(HC[0], grid, grid, grid, grid, distances)
        grid = grid.unsqueeze(1)
        distances = distances.unsqueeze(1)
        bpred3 = bspline3(grid[N1], grid[N2], grid[N3], grid[N4], distances[Nd])
        bpred32 = bspline3(torch.tensor([2.5, 2.75, 3.0, 3.75]), grid[N2], grid[N3], grid[N4], distances[Nd])


        print('bpred3', bpred3, bpred32)
        x = torch.linspace(1, N-1, N-1) * 0.2
        # plt.plot(ref[1:N], ref[1:N], label='skgen reference')
        # plt.plot(x, ref[1:N]-y.squeeze()[1:N])
        ...
        # plt.plot(x, ref3[1:N]-bpred3[0], label='bspline H-C')
...
class TensorSpline:
    ...
    def __init__(self, f_nodes: torch.Tensor, x_nodes: torch.Tensor,
        ...
        if self.f_nodes.dim() == len(self.nodes) + 1:
            ...
            raise ValueError('can not support current f_nodes dims')
