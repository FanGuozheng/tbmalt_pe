...
class VcrSkf(Skf):
    ...
    def __init__(
            self, atom_pair: Tensor,
            hamiltonian: SkDict, overlap: SkDict,
            grid: Tensor, r_spline: Optional[RSpline] = None,
            r_poly: Optional[RPoly] = None, hubbard_us: OptTens = None,
            on_sites: OptTens = None, occupations: OptTens = None,
            mass: OptTens = None):
        super().__init__(atom_pair,
                         hamiltonian, overlap,
                         grid, r_spline, r_poly, hubbard_us,
        ...
        if isinstance(path, list):
            ...
            for ie, idx in zip(element_set, ind):
                ...
                if _iskf[-1].r_spline is not None:
                    ...
                    r_cutoff = pack([ii.r_spline.cutoff for ii in _iskf])
                ...
                _skf.append(cls(atom_pair, h_data, s_data, grid, **kwargs_in))
