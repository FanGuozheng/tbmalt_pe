...
from tbmalt.ml.optim import OptHs, OptCompressionR
...
compr_grid = torch.tensor([01.00, 01.50, 02.00, 02.50, 03.00, 03.50, 04.00,
...
def optimize(dataset_ref, size, dataset_dftb=None, **kwargs):
    ...
    params['ml']['lr'] = 0.001 if params['ml']['task'] == 'mlIntegral' else 0.05

    geo_opt, data_ref = _load_ref(dataset_ref, size, ['charge', 'dipole', 'hirshfeld_volume_ratio'])
    ...
    elif params['ml']['task'] == 'mlCompressionR':
        params['dftb']['path_to_skf'] = './slko/skf_comprwav.hdf'
        params['dftb']['path_to_skf2'] = '../../slko/mio-1-1/'
        opt = OptCompressionR(geo_opt, data_ref, params, compr_grid, shell_dict,
...
def _load_ref(dataset, size, properties, units='angstrom'):
    ...
    geo = Geometry(numbers, positions, units=units)
