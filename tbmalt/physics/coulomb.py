...
class Ewald(ABC):
    ...
    def __init__(self, geometry: object, periodic: object, param, **kwargs):
        ...
        atom_nums = self.geometry.atomic_numbers.unsqueeze(0) if dim == 1 \
            else self.geometry.atomic_numbers
        self.coord = self.geometry.positions.unsqueeze(0) if dim == 1 \
            else self.geometry.positions
        self.natom = atom_nums.ne(0).sum(-1)
        ...
        self._n_batch = self.geometry._n_batch if dim == 2 else 1
