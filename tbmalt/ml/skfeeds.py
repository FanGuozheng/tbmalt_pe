...
class SkfFeed(_SkFeed):
    ...
    def from_dir(cls, path: str,
        ...
        # elif interpolation == 'ToyInterp2D':
        #     interpolator = ToyInterp2D
        else:
            raise NotImplementedError('%s is not implemented.' % interpolation)

        if (is_dir := os.path.isdir(path)):
            warn('"hdf" binary Slater-Koster files are suggested.')
        ...
        if skf_type == 'skf':
            warn('"h5" is suggested for "skf_type".')
        elif skf_type == 'h5':
...
def _get_hs_dict(hs_dict: dict, interpolator: object,
    ...
    """
    hs_grad = kwargs.get('s_grad', False)
    ...
    for ii, interaction in enumerate(hs_data.keys()):
        if vcr is None:
            hs_dict[(*skf.atom_pair.tolist(), *interaction)] = interpolator(
                skf.grid, hs_data[interaction].T.requires_grad_(hs_grad))
        else:
            hs_dict[(*skf.atom_pair.tolist(), *interaction)] = interpolator(
                vcr, vcr, skf.grid, hs_data[interaction].requires_grad_(hs_grad))
